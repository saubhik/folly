...
static R wrapSocketFunction(F f, ShNetworkSocket s, Args... args) {
  R ret = f(s.data, args...);
#ifdef _WIN32
...
#else
  return ShNetworkSocket(wrapSocketFunction<ShNetworkSocket::native_handle_type>(
...
ssize_t sendmsg(ShNetworkSocket socket, const msghdr* message, int flags) {
  ...
  for (size_t i = 0; i < message->msg_iovlen; i++) {
    ...
    if (message->msg_name != nullptr) {
      r = sock->WriteTo(
          (const void*) message->msg_iov[i].iov_base,
          (size_t) message->msg_iov[i].iov_len,
          (const netaddr*) message->msg_name
      );
    } else {
      r = sock->Write(
          (const void*) message->msg_iov[i].iov_base,
...
ShNetworkSocket socket(int af, int type, int protocol) {
#if defined(__XROS__)
  throw std::logic_error("Not implemented!");
#else
  // return ShNetworkSocket(::socket(af, type, protocol));
...
int socketpair(int domain, int type, int protocol, ShNetworkSocket sv[2]) {
  ...
  sv[0] = ShNetworkSocket(static_cast<SOCKET>(pair[0]));
  sv[1] = ShNetworkSocket(static_cast<SOCKET>(pair[1]));
  return r;
#elif defined(__XROS__)
  throw std::logic_error("Not implemented!");
#else
  ...
  //   return r;
  // }
  // sv[0] = ShNetworkSocket(pair[0]);
  // sv[1] = ShNetworkSocket(pair[1]);
  // return r;
  throw std::logic_error("Not implemented!");
#endif
}
