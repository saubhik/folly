...
class UDPAcceptor : public ShenangoAsyncUDPServerSocket::Callback {
 public:
  ...
  void onDataAvailable(
      std::shared_ptr<folly::ShenangoAsyncUDPSocket> /* socket */,
...
class UDPClient
    : private ShenangoAsyncUDPSocket::ReadCallback, private AsyncTimeout {
 public:
...
class UDPNotifyClient : public UDPClient {
 public:
  ...
  void onRecvMsg(ShenangoAsyncUDPSocket& sock) {
    ...
    SocketAddress addr = folly::SocketAddress(rt::NetaddrToIPString(address),
                                              address.port);
