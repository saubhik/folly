...
ssize_t ShenangoAsyncUDPSocket::writeChain(
...
ssize_t ShenangoAsyncUDPSocket::writev(
    const folly::SocketAddress& address,
...
int ShenangoAsyncUDPSocket::writem(
...
int ShenangoAsyncUDPSocket::writeImpl(
    Range<SocketAddress const*> addrs,
    const std::unique_ptr<folly::IOBuf>* bufs,
    size_t count,
...
int ShenangoAsyncUDPSocket::recvmmsg(
    struct mmsghdr* msgvec,
    unsigned int vlen,
    unsigned int flags,
...
void ShenangoAsyncUDPSocket::handleRead() noexcept {
  ...
  while (numReads-- && readCallback_ && eventBase_ == originalEventBase) {
    ...
    if (bytesRead >= 0) {
      clientAddress_ = folly::SocketAddress(rt::NetaddrToIPString(addr),
                                            addr.port);
